...
```bash
imagehive doctor     # host, install, models, service, clients — with exit status
imagehive status     # resident tier, loads, queue, peak memory
imagehive logs       # last 40 lines of the daemon log
```
...
Ctrl-C and re-run `imagehive models pull <preset>`: finished files are
...
from. `imagehive models` shows what is there; `imagehive models pull
...
**`imagehive: command not found` after installing.**
`~/.local/bin` is not on the default macOS `PATH`. Either call it by full path
(`~/.local/bin/imagehive doctor`) or add `export PATH="$HOME/.local/bin:$PATH"`
...
artifact (`imagehive models pull fast-4bit`) if you want that path, then
`imagehive restart`. `available_tiers` in `imagehive status` lists what the
daemon can serve.
...
`imagehive`. If the entry is missing entirely, `imagehive clients add <name>`.
...
if you would rather stay warm, or call `imagehive unload` when you are done.
...
`launchctl print gui/$(id -u)/<label>` — `local.imagehive` unless you installed
with `--label` — while every answer still
comes from the previous build. Installing, `imagehive stop` and
`imagehive restart` all end whatever holds the socket, so start there —
`imagehive restart`, then `imagehive status`, which names the process and
...
so if a value changes nothing, check whether `IMAGEHIVE_TTL_SECONDS` and friends
are set in the launchd job or in the client entry that starts the daemon.

**`imagehive status` says the daemon is unreachable, but the socket file is there.**
...
up unlinks a socket nobody is listening on before it binds, and `imagehive stop`
...
are loaded. `imagehive options` lists the recommended sizes (1024×1024,
...
really want a fresh daemon: `imagehive restart`.

**A download stopped halfway.**
Just re-run `imagehive models pull <preset>` — it resumes and skips files
whose size already matches. `imagehive models verify` compares the files on
...
```bash
HF_ENDPOINT=https://hf-mirror.com imagehive models pull fast-4bit hf
```

## Collecting a useful bug report

```bash
imagehive doctor > /tmp/doctor.txt 2>&1; echo "exit=$?" >> /tmp/doctor.txt
imagehive status >> /tmp/doctor.txt 2>&1
tail -100 ~/Library/Logs/ImageHive/imagehived.log >> /tmp/doctor.txt
